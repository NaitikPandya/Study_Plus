...
class ClassFile extends StatefulWidget {
  String className;
  String classCode;
  ClassFile(className,classCode){this.className=className;this.classCode=classCode;}
  @override
 ...
 int vTask;
  ...
  void initState() {
    ...
    .getDocuments().then((doc){
      setState(() {
        assignment = Firestore.instance.document("Classes/${doc.documents[0].documentID}").collection("Assignments").snapshots();
        video = Firestore.instance.document("Classes/${doc.documents[0].documentID}").collection("video").snapshots();
        student = Firestore.instance.document("Classes/${doc.documents[0].documentID}").collection("Student").snapshots();
        Firestore.instance.document("Classes/${doc.documents[0].documentID}").collection("Assignments").snapshots().listen((onData){
          int i = onData.documents.length;
          aTask = i;
        });
        Firestore.instance.document("Classes/${doc.documents[0].documentID}").collection("video").snapshots().listen((onData){
      ...
      child: Scaffold(
        ...
        body: TabBarView(
          children: <Widget>[
            ...
            Scaffold(
              body: load ? Center(child:SpinKitChasingDots(color:Colors.brown))
                  : GestureDetector(
            ...
            itemBuilder: (BuildContext context, int index) {
              return Card(
                child: Column(
                  children: <Widget>[
                    ListTile(
                      title: Text("Video Lec",style: TextStyle(fontSize: 20.0,fontWeight: FontWeight.bold),),
                      subtitle: Text(snapshot.data.documents[index].data['Title']),
                    ),
                    Align(
                      alignment: Alignment.center,
                      child: Icon(Icons.personal_video,size:50.0,color: Colors.grey[500],),
                    ),
                    Padding(padding: EdgeInsets.only(bottom: 40.0),),
                    Container(
                      height: 40.0,
                      child: RaisedButton(
                        onPressed: () async {
                          ...
                          StorageReference store = FirebaseStorage.instance
                              ...
                              .child(filepath)
                              .child("Videos")
                              .child("${snapshot.data.documents[index].data['Title']}");
                          String path = await store.getDownloadURL();
                          _launchUniversalLinkIos(path.toString());
                        },
                        child: Row(
    ...
    if(name != null)
      {
        return ListView(
          children: <Widget>[
            Center(
              child: Text("Class Details",style: TextStyle(fontSize: 25.0),),
            ),
                  ...
                  Column(
                    ...
                    Card(
                      ...
                      child: Column(
                        children: <Widget>[
                          Center(
                            child: Text("Teacher Details",style: TextStyle(fontSize: 20.0),),
                          ),
                        ListTile(
                          leading: CircleAvatar(
            ...
            Padding(padding: EdgeInsets.only(bottom: 10.0),),
            Center(
              child: Text("Student List",style: TextStyle(fontSize: 25.0),),
            ),
            _student()
          ],
        );
          ...
          stream: student,
            builder: (context , snapshot)
            {
              ...
              return ListView.builder(
                  ...
                  itemCount: snapshot.data.documents.length,
                  itemBuilder: (BuildContext context, int index)  {
                        Firestore.instance.document("user/${snapshot.data.documents[index].data['uid']}").snapshots()
                            .listen((onData){
                           setState(() {
                    ...
                    return  Card(
                          child: ListTile(
                            ...
                            title: Student != null ? Text(Student): Text("Loading...."),
